...
public class AStarPathfinder {
    ...
    public static Waypoint computePath(Map2D map) {
        ...
        while (!foundPath && state.numOpenWaypoints() > 0) {
            ...
            /*Add/update all neighbors of the current best location.
              This is equivalent to trying all "next steps" from this location.*/
            takeNextStep(best, state);
            // Finally, move this location from the "open" list to the "closed" list.
    ...
    private static void takeNextStep(Waypoint currWP, AStarState state) {
        ...
        for (int y = loc.yCoord - 1; y <= loc.yCoord + 1; y++) {
            for (int x = loc.xCoord - 1; x <= loc.xCoord + 1; x++) {
                ...
                if (nextLoc == loc)
                    continue;
                // If this location happens to already be in the "closed" set then continue on with the next location.
